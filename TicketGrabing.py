...
def BuyTicket():
    ...
    try:
        wait = WebDriverWait(driver, 5)
        wait.until(EC.presence_of_element_located(
            (By.XPATH, '//*[@id="calendar"]/table/tbody/tr[2]/td[6]'))).click()
        wait = WebDriverWait(driver, 5)
        wait.until(EC.presence_of_element_located(
            (By.XPATH, '//*[@id="PerformanceListTable"]/table/tbody/tr[2]/td[5]'))).click()
        wait = WebDriverWait(driver, 5)

        wait.until(EC.presence_of_element_located(
            (By.XPATH, '/html/body/div[5]/div[4]/div[2]/div/div[3]/div/div/table/tbody')))
        # elememt = driver.find_elements('xpath','/html/body/div[5]/div[4]/div[2]/div/div[3]/div/div/table/tbody/tr[2]')
        rows = driver.find_elements(
            By.XPATH, '/html/body/div[5]/div[4]/div[2]/div/div[3]/div/div/table/tbody/tr')
        # print(len(rows))
        for row in rows:
            colums = row.find_elements(By.TAG_NAME, 'td')
